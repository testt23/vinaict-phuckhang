...
class User extends User_model {
    ...
    function getList(&$filter = array()) {
        ...
        if(isset($filter['name']) && $filter['name']) {
            $user->addWhere("user.last_name LIKE '%".$filter['name']."%' OR user.first_name LIKE '%".$filter['name']."%'");
        }
        ...
        // Get total rows
        $user->addSelect();
        $user->addSelect('COUNT(*) count');
        $user->find();
        $user->fetchNext();
        
        // Initialize pagination
        $this->load->library('pagination');
        $this->pagination->total_rows = $user->count;
        ...
        $user->limit($this->pagination->first_row .", ".$this->pagination->rows_per_page);
        $user->find();
    ...
    {
        $acces = new User();
        $acces->addWhere("email = '" . mysql_real_escape_string($login) . "'");
        $acces->addWhere("pass = '" . sha1(mysql_real_escape_string($pwd)) . "'");

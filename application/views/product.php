    ...
    <table> 
        ...
        <tbody> 
            ...
            <tr <?php if ($product->is_disabled == IS_DISABLED) { ?>class="row-disabled"<?php } ?> >
                ...
                <td><?php if ($product->picture) { ?><a href="<?php echo config_item('source_image').$image_group_code.'/'.$product->picture; ?>" title="<?php echo clean_html(getI18n($product->short_description)); ?>" rel="prettyPhoto" ><img src="<?php echo config_item('source_image').$image_group_code.'/'.str_replace(array('.jpg','.png','.gif','.JPG','.PNG','.GIF'), array(BO_PROD_IMG_SUFFIX.'.jpg',BO_PROD_IMG_SUFFIX.'.png',BO_PROD_IMG_SUFFIX.'.gif',BO_PROD_IMG_SUFFIX.'.JPG',BO_PROD_IMG_SUFFIX.'.PNG',BO_PROD_IMG_SUFFIX.'.GIF'), $product->picture); ?>" alt="<?php echo $product_name; ?>" /></a><?php } else { echo lang('txt_no_picture'); } ?></td>

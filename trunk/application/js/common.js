 ...
 */
 ...
 function showMessageBubble(control, msg, seconds)
 {
    var delay = seconds || 5000;
        ...
        switch(opts.type){
            case 'checkbox':
                ...
                item.wrap('<div />').after(
                    wrap = $('<span />').append(
                        '&nbsp;&nbsp;',
                        valid = $('<a href="javascript:void(0);" />').html('<img src="'+site_url+'/images/done_small.png" style="vertical-align:-2px" />')
                    ).attr({
                        style: 'display:none;'
                    })
                );
            ...
            default: // text
                ...
                item.wrap('<div />').after(
                    ...
                    opts.type != 'checkbox' ?
                        ...
                        .html('<img src="'+site_url+'/images/edit.png" style="vertical-align:-2px" />') : '',
                    wrap = $('<span />').append(
                        input, '&nbsp;&nbsp;',
                        valid = $('<a href="javascript:void(0);" />')
                        .attr({
                            id: "save_bid_"+opts.data.id
                        }).html('<img src="'+site_url+'/images/done_small.png" style="vertical-align:-2px" />'),
                        cancel = $('<a href="javascript:void(0);" />')
                        .attr({
                            id: "cancel_bid_"+opts.data.id
                        })
                        .html('<img src="'+site_url+'/images/delete_small.png" style="vertical-align:-2px" />')
                    ).attr({
                        style: 'display:none;'
                    })
                );
...
function switchLang(lang, name) {      
      ...
      for (i in lang_iso) {
          ...
          if ($('#'+name+'_'+lang_iso[i]).attr('class') == 'ckeditor') {
            $('#ckeditor-container_'+name+'_'+lang_iso[i]).hide();
          }
          else {
              $('#'+name+'_'+lang_iso[i]).hide();
          }
      ...
      if ($('#'+name+'_'+lang).attr('class') == 'ckeditor') {
          $('#ckeditor-container_'+name+'_'+lang).show();
      }
      else {
          $('#'+name+'_'+lang).show();
      }
...
$(document).ready(function() {
   ...
   $('div[type=text-multilang]').each(function(){
       ...
       for (var i = 0; i < arr_lang.length; i++) {
           $(this).append('<a id="btn_'+$(this).attr('name')+'_'+arr_lang[i]+'" href="javascript:switchLang(\''+arr_lang[i]+'\', \''+$(this).attr('name')+'\')" class="lang-icon" '+(arr_lang[i] == def_lang ? 'style="opacity:1"' : 'style="opacity:0.3"')+'><img src="'+site_url+'images/icons/'+arr_lang[i]+'.png" alt="'+lang_name[i]+'" /></a>');
       }
   ...
   $('div[type=texteditor-multilang]').each(function(){
       ...
       for (var i = 0; i < arr_lang.length; i++) {
           $(this).append('<div style="display:'+(arr_lang[i] == def_lang ? 'block;' : 'none;')+'" class="ckeditor-container" id="ckeditor-container_'+$(this).attr('id')+'_'+arr_lang[i]+'"><textarea class="ckeditor" id="'+$(this).attr('id')+'_'+arr_lang[i]+'" name="'+$(this).attr('name')+'_'+arr_lang[i]+'" style="float:left;display:'+(arr_lang[i] == def_lang ? 'block;' : 'none;')+'">'+getI18n($(this).attr('value'), arr_lang[i])+'</textarea></div>');
       }

...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('.ui-icon-arrowreturnthick-1-w, .ui-icon-cancel').hover( 
            function(){
        ...
        $('.ui-icon-clock').hover( 
            function(){
                $(this).addClass('ui-icon-transferthick-e-w');
            },function(){
    ...
    <table> 
        ...
        <tbody> 
            ...
            <tr>
                ...
                <td><?php $order_name = $order->is_business == 1 ? clean_html(getI18n($order->company)) : $order_name = clean_html(getI18n($order->lastname. '' . $order->firstname)); echo $order_name;  ?></td>                
                <td><?php echo $order->description; ?></td>
                <td><?php if($order->order_date == NULL) echo "Null"; else echo date('d-m-Y',strtotime($order->order_date)); ?></td>
                <td><?php echo $order->amount ? $order->amount : lang('txt_call') ; ?></td>
                <td><?php $status = $order->status;  echo clean_html(getI18n($list_status[$status])); ?></td>
                <td><?php if($order->shipping_date == Null) echo "Null"; else echo date('d-m-Y',strtotime($order->shipping_date)); ?></td>
                <td>
                    ...
                    <?php $status_next = $status + 1; $status_prev = $status - 1; if($status == 4){ ?>

...
(function(jQuery) {
	...
	jQuery.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			...
			imageLoading:			'<?php echo base_url(); ?>images/site/loadding.gif',		// (string) Path and the name of the loading icon
			imageBtnPrev:			'<?php echo base_url(); ?>images/site/prev.gif',			// (string) Path and the name of the prev button image
			imageBtnNext:			'<?php echo base_url(); ?>images/site/next.gif',			// (string) Path and the name of the next button image
			imageBtnClose:			'<?php echo base_url(); ?>images/site/close.gif',		// (string) Path and the name of the close btn
			imageBlank:			'<?php echo base_url(); ?>images/site/blank.gif',			// (string) Path and the name of a blank image (one pixel)
			// Configuration related to container image box
			containerBorderSize:	10,			// (integer) If you adjust the padding in the CSS for the container, #lightbox-container-image-box, you will need to update this value
			containerResizeSpeed:	400,		// (integer) Specify the resize duration of container image. These number are miliseconds. 400 is default.
			// Configuration related to texts in caption. For example: Image 2 of 8. You can alter either "Image" and "of" texts.
			txtImage:				'<?php echo lang('image'); ?>',	// (string) Specify text "Image"
		...
		function _set_image_to_view() { // show the loading
			...
			objImagePreloader.onload = function() {
			    //Binh Nguyen
			    //alert(document.documentElement.clientWidth +"---"+ objImagePreloader.width);

                if(document.documentElement.clientWidth > objImagePreloader.width &&
                document.documentElement.clientHeight*80/100 > objImagePreloader.height)
			    {
				...
				else
				{
				    ...
				    //var nPercent;
				    //nPercent = (document.documentElement.clientWidth > objImagePreloader.width?document.documentElement.clientWidth / objImagePreloader.width:document.documentElement.clientHeight / objImagePreloader.height);
				    jQuery('#lightbox-image').attr('src',settings.imageArray[settings.activeImage][0]);
				    //jQuery('#lightbox-image').attr('width', Math.round(objImagePreloader.width * nPercent-50));
				    //jQuery('#lightbox-image').attr('height', Math.round(objImagePreloader.height * nPercent-50));
				    if(objImagePreloader.width>document.documentElement.clientWidth)
				    jQuery('#lightbox-image').attr('width', Math.round(document.documentElement.clientWidth * 80/100));
				    else
				    jQuery('#lightbox-image').attr('width', Math.round(objImagePreloader.width * 80/100));
				    
				    jQuery('#lightbox-image').attr('height', Math.round(document.documentElement.clientHeight * 80/100));
				    // Perfomance an effect in the image container resizing it
				    //_resize_container_image_box(Math.round(objImagePreloader.width * 70/100),Math.round(objImagePreloader.height * 70/100));
				    if(objImagePreloader.width>document.documentElement.clientWidth)
				    _resize_container_image_box(Math.round(document.documentElement.clientWidth * 80/100),Math.round(document.documentElement.clientHeight * 80/100));
				    else
				    _resize_container_image_box(Math.round(objImagePreloader.width * 80/100),Math.round(document.documentElement.clientHeight * 80/100));
				    //	clear onLoad, IE behaves irratically with animated gifs otherwise
				}
				objImagePreloader.onload=function(){};
			};
		...
		function _show_image_data() {
			...
			if ( settings.imageArray.length > 1 ) {
                            jQuery('#lightbox-image-details-currentNumber').html(settings.txtImage + ' ' + ( settings.activeImage + 1 ) + ' ' + settings.txtOf + ' ' + settings.imageArray.length + ' <?php echo Variable::getDomainName(); ?>').show();
			}		
		}
		/**
